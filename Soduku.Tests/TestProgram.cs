...
namespace Soduku.Tests
{
    ...
    public class TestProgram
    {
        ...
        {
            var arr = new int[81];
            Assert.That(Program.GetNextTestCell(arr), Is.EqualTo(0));

            Enumerable.Range(0, 27).Select(c => 1).ToArray().CopyTo(arr, 0);
            Assert.That(Program.GetNextTestCell(arr), Is.EqualTo(27));

            Enumerable.Range(0, 81).Select(c => 1).ToArray().CopyTo(arr, 0);
            Assert.That(Program.GetNextTestCell(arr), Is.EqualTo(81));
        }
        ...
        public void TestGetBlock()
        {
            ...
            Assert.That(Program.GetBlock(arr, 0, 0), Is.EqualTo(new[] {0, 1, 2, 9, 10, 11, 18, 19, 20}));
            Assert.That(Program.GetBlock(arr, 0, 1), Is.EqualTo(new[] {3, 4, 5, 12, 13, 14, 21, 22, 23}));
            Assert.That(Program.GetBlock(arr, 0, 2), Is.EqualTo(new[] {6, 7, 8, 15, 16, 17, 24, 25, 26}));

            Assert.That(Program.GetBlock(arr, 1, 0), Is.EqualTo(new[] {27, 28, 29, 36, 37, 38, 45, 46, 47}));
            Assert.That(Program.GetBlock(arr, 1, 1), Is.EqualTo(new[] {30, 31, 32, 39, 40, 41, 48, 49, 50}));
            Assert.That(Program.GetBlock(arr, 1, 2), Is.EqualTo(new[] {33, 34, 35, 42, 43, 44, 51, 52, 53}));

            Assert.That(Program.GetBlock(arr, 2, 0), Is.EqualTo(new[] {54, 55, 56, 63, 64, 65, 72, 73, 74}));
            Assert.That(Program.GetBlock(arr, 2, 1), Is.EqualTo(new[] {57, 58, 59, 66, 67, 68, 75, 76, 77}));
            Assert.That(Program.GetBlock(arr, 2, 2), Is.EqualTo(new[] {60, 61, 62, 69, 70, 71, 78, 79, 80}));
        }
        ...
        public void TestIsBlockValid()
        {
            for (int rb = 0; rb < 3; rb++)
            {
                for (int cb = 0; cb < 3; cb++)
                {
                    ...
                    Assert.That(Program.IsBlockValid(arr, rb, cb), Is.True);
                    arr[pos] = 2;
                    Assert.That(Program.IsBlockValid(arr, rb, cb), Is.False);
                }
            }
        }
        ...
        public void TestAreBlocksValid()
        {
            ...
            Assert.That(Program.AreBlocksValid(arr), Is.True);

            arr[0] = 1;
            Assert.That(Program.AreBlocksValid(arr), Is.False);
        }
        [Test]
        public void TestIsValid()
        {
            ...
            Assert.That(Program.IsValid(arr), Is.True);

            arr[0] = 1;
            Assert.That(Program.IsValid(arr), Is.False);
        }
    }
}
